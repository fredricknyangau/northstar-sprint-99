...
const checkReturnButton = document.getElementById("checkReturnButton");
const returnStatusResult = document.getElementById("returnStatusResult");
...
checkOrderButton.addEventListener("click", async function () {
  ...
  try {
    ...
    if (!response.ok) {
      orderResult.textContent = "Something went wrong. Please try again.";
      orderResult.classList.add("show");
      return;
    }
...
submitReturnButton.addEventListener("click", async function () {
  const orderId = returnOrderIdInput.value.trim();
  const reason = returnReasonInput.value.trim();

  if (orderId === "") {
    returnResult.textContent = "Please enter an Order ID.";
    returnResult.classList.add("show");
    return;
  }

  if (reason === "") {
    returnResult.textContent = "Please enter a reason for the return.";
    returnResult.classList.add("show");
    return;
  }
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/returns`, {
      ...
      body: JSON.stringify({ order_id: parseInt(orderId), reason: reason }),
    });

    if (response.status === 404) {
      returnResult.textContent = "Order not found. Please check the Order ID.";
      returnResult.classList.add("show");
      return;
    }

    if (!response.ok) {
      returnResult.textContent = "Something went wrong creating the return.";
      returnResult.classList.add("show");
      return;
    }
...
checkReturnButton.addEventListener("click", async function () {
  const returnId = returnIdInput.value.trim();

  if (returnId === "") {
    returnStatusResult.textContent = "Please enter a Return ID.";
  ...
  try {
    ...
    if (response.status === 404) {
      returnStatusResult.textContent = "Return not found. Please check the ID.";
      returnStatusResult.classList.add("show");
      return;
    }
...
classifyButton.addEventListener("click", async function () {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/support/classify`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    const intentLabel = labelMap[data.category] || data.category;
...
const supportHome = document.getElementById("supportHome");
